...
(function ($, F) {
	F.Filtering = F.Component.extend(/** @lends FooTable.Filtering */{
		...
		$create: function () {
			...
			self.$input = $('<input/>', {type: 'text', 'class': 'form-control', placeholder: self.placeholder,style:'width:150px;height:30px;border-radius:2px 0px 0px 2px;'});
...
(function ($, F) {
	F.Sorting = F.Component.extend(/** @lends FooTable.Sorting */{
		...
		draw: function () {
			...
			$(".table thead tr:nth-child(2)").children('th:first').children("span:first").css("display","none");
		},
...
(function($, F){
	F.Paging = F.Component.extend(/** @lends FooTable.Paging */{
		...
		_onPageClicked: function(e){
			...
			switch(page){
				case 'first': self.first(); setTimeout(updateUI,700); 
					return;
				case 'prev': self.prev(); setTimeout(updateUI,700); 
					return;
				case 'next': self.next(); setTimeout(updateUI,700); 
					return;
				case 'last': self.last(); setTimeout(updateUI,700); 
					return;
				case 'prev-limit': self.prevPages(); setTimeout(updateUI,700); 
					return;
				case 'next-limit': self.nextPages(); setTimeout(updateUI,700); 
					return;
				default: self._set(page); setTimeout(updateUI,700); 
					return;
			}
		}
	});
