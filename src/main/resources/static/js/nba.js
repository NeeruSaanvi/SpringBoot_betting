 ...
 $(document).ready(function(){
  ...
  $(document).ready(function(){
	  ...
	  sessionStorage.setItem('sport','nba');
 ...
 function initializeUI(sport,teamName){
	 var url = "/playersinfo/info/v1/teamplayers?team="+teamName+"&sport="+sport+"&game=game&line=spread&sportsbook=BOOKMAKER LINE MOVEMENTS";
		//'sportteam/'+sport+"/"+teamName
	 	...
	 	var playerTeam;
	 	var topTblData;
		$.fn.requestCall('GET','',url,40000,function(data){
			console.log("DATA: "+data);
			var resObj = JSON.parse(data);
			playerTeam = resObj.playersData;
			topTblData = resObj.topTableData;//[0]['detail'];
			 ...
			 for(var k in obj) {
				 ...
				 if(obj[k] instanceof Object) {
					 var hoverstr = "RPG "+obj[k].rpg+", PPG "+obj[k].ppg+", APG "+obj[k].apg+", PER "+obj[k].per;
							...
							trs += "<td class='a'>"+Math.round(dRating/counter)+"</td>"
							trs += "<td class='a'>"+Math.round(oRating/counter)+"</td>"
		...
		function createTableRow(topTableData, key,showKey,avrgJson,isExistsAvrgKey){
			...
			/*if(isExistsAvrgKey && key=='overUnder') {
				(overtimemin/overtimecounter).toFixed(1)
			}*/
	...
	/*	function createTableRow(topTableData, key,showKey,avrgJson,isExistsAvrgKey){
			...
			if(isExistsAvrgKey)
				avrgJson[showKey]=avrg/topTableData.length;
			return "<tr class='a'>"+rowCols+"</tr>";
			 var date= rowObj.date!=null ? rowObj.date : "-";
			 var opponent= rowObj.opponent!=null ? rowObj.opponent : "-";
			 var result= rowObj.result!=null ? rowObj.result : "-";
			 var points= rowObj.points!=null ? rowObj.points : "-";
			 var oppPoints= rowObj.oppPoints!=null ? rowObj.oppPoints : "-";
			 var totalPoints= rowObj.totalPoints!=null ? rowObj.totalPoints : "-";
			 var closingSpread= rowObj.closingSpread!=null ? rowObj.closingSpread : "-";
			 var atsResult= rowObj.atsResult!=null ? rowObj.atsResult : "-";
			 var sideCoverMargin= rowObj.sideCoverMargin!=null ? rowObj.sideCoverMargin : "-";
			 var closingTotal= rowObj.closingTotal!=null ? rowObj.closingTotal : "-";
			 var overUnder= rowObj.overUnder!=null ? rowObj.overUnder : "-";
			 var totalCoverMargin= rowObj.totalCoverMargin!=null ? rowObj.totalCoverMargin : "-";
			 var overtime= rowObj.overtime!=null ? rowObj.overtime : "-";
			 var offRating= rowObj.offRating!=null ? rowObj.offRating : "-";
			 var defRating= rowObj.defRating!=null ? rowObj.defRating : "-";
			 var possessions= rowObj.possessions!=null ? rowObj.possessions : "-";
			 ...
			 var rowStr = "<tr>" +
			 					...
			 					"<td>"+closingSpread+"</td>" +
			 					"<td>"+atsResult+"</td>" +
			 					"<td>"+sideCoverMargin+"</td>" +
			 					"<td>"+closingTotal+"</td>" +
			 					"<td>"+totalCoverMargin+"</td>" +
			 					"<td>"+overtime+"</td>" +
			 					"<td>"+offRating+"</td>" +
			 					"<td>"+defRating+"</td>" +
			 					"<td>"+possessions+"</td>" +
